/**
 * FORCE SYNC GLOBALINDEX - Browser Console Version
 * 
 * CARA PAKAI:
 ...
 * 5. Tekan Enter
 * 6. Script akan otomatis sync semua produk dari batch_1 ke globalindex
 */

(async function forceSyncGlobalIndexFromConsole() {
  ...
  try {
    // Import Firebase dari window object (sudah loaded di app)
    const { db, collection, doc, getDoc, setDoc, getDocs } = window;
    
    if (!db) {
      console.error('❌ Firebase not loaded! Make sure you are on the app page and logged in.');
      return;
    }
