// Address API with Firestore Caching - TEMPORARY ENDPOINT
// Based on working shipping cache implementation (cost-cached.js)
// This should work with the same API key that writes to shipping_cache
...
// Firebase cache functions - Based on working shipping cache implementation
...
async function setCachedData(collectionName, documentId, data, ttlHours) {
  try {
    ...
    // Using the same structure as shipping cache that works
    const firestoreData = {
      fields: {
...
export default async function handler(req, res) {
  try {
    ...
    console.log(`📍 Address API TEMP request: type=${type}, provinceId=${provinceId}, cityId=${cityId}, districtId=${districtId}`);
    ...
    console.log(`🔍 Checking TEMP cache for ${expectedType}: ${cacheKey}`);

    // Try to get from cache first
    let dataArray = await getCachedData(cacheCollection, cacheKey);

    if (dataArray) {
      console.log(`✅ ${expectedType} loaded from TEMP cache (${dataArray.length} items)`);
      return res.status(200).json({
        success: true,
        message: `${expectedType} loaded from TEMP cache`,
        data: dataArray
      });
    }
    ...
    if (dataArray.length > 0) {
      ...
      console.log(`💾 Saving ${expectedType} TEMP cache with TTL: ${ttl} hours`);
    ...
    return res.status(200).json({
      success: true,
      message: `${expectedType} loaded successfully (TEMP MODE)`,
      data: transformedData
    });

  } catch (error) {
    console.error('💥 Address API Error (TEMP MODE):', {
    ...
    return res.status(500).json({
      success: false,
      message: `Address API error (TEMP MODE): ${error.message}`,
      data: null
    });
  }
}